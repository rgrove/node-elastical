...
var thrift = require('thrift'),
    ...
    Rest = require('./gen-nodejs/Rest'),
    ttypes = require('./gen-nodejs/elasticsearch_types');
...
function Client(host, options) {
    ...
    if (this.options.port == 9500) {
        this.options.protocol = 'thrift';
        this._connection = thrift.createConnection(
            this.options.host, this.options.port, 
            {transport: ttransport.TBufferedTransport, timeout: this.options.timeout}
        );
        this._thriftClient = thrift.createClient(Rest, this._connection);
...
Client.prototype = {
    ...
    **/
    _request: function (path, options, callback) {
        ...
        if (this.options.protocol == 'thrift') {
            var thriftRequest = new ttypes.RestRequest({
                method: ttypes.Method[options.method],
                uri: path,
                body: JSON.stringify(options.json)
            });
            
            this._thriftClient.execute(thriftRequest, function(err, response) {
                if (err) { return callback(err), undefined; }
                
                body = JSON.parse(response.body);
                if (response.status < 200 || response.status > 299) {
                    callback(Error((body && body.error) || 'Thrift ' + response.status),
                        body || {});
                    return;
                }
