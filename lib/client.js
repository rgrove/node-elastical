...
Client.prototype = {
    ...
    **/
    ...
    /**
    Registers a percolator for the given index and
    ...
    @example
        ...
        client.createPercolator('pecolatorId', 'tweets', query, function (err, res) {
            if (err) { throw err; }
            console.log(results);
        });
    @method percolator
    ...
    @param {String} percolatorId The identifier string of the percolator.
    ...
    @param {Object|String} [query] Search query. This can be a simple
          string (in which case a "query_string" search will be performed) or a
          full query object. See the [ElasticSearch Query DSL docs](http://www.elasticsearch.org/guide/reference/query-dsl/)
          for details.
    @param {Function} callback Callback function.
      ...
      @param {Object} callback.res Full ElasticSearch response data.
    */
    addPercolator: function(index, percolatorId, query, callback){
        // validate the input arguments
        if(typeof callback !== 'function'){
            throw new Error("callback function cannot be null!");
        }
        if(typeof index === undefined || index == null || index == ''){
            var err = new Error('index name is null');
            callback(err, null);
        }
        if(typeof percolatorId === undefined || percolatorId == null || percolatorId == ''){
            var err = new Error('percolatorId is null');
            callback(err, null);
        }
        ...
        var url = '/_percolator/';
        url += encode(Array.isArray(index) ? index.join(',') : index);
        url += '/'+pecolatorId;
        this._request(url, {
            method: 'POST',
            json  : query
        }, function (err, res) {
            if (err) { return callback(err, null), undefined; }
     ...
     * @example
     *  var client = new require('elastical').Client();
     *  client.getPercolator(index, percoloatId, function (err, results) {
     *      if (err) { throw err; }
     *       console.log(results);
     *  });
     *
     *  @param {String} index  The index name to which the percolator is registered
     *  @param {String} percolatorID The name of the percolator
     *  @param {Function} callback Callback function.
     *      @param {Error|null} callback.err Error, or `null` on success.
     *      @param {Object} callback.results  Percolator document.
     *      @param {Object} callback.res Full ElasticSearch response data.
     */     
    getPercolator: function(index, percolatorId, callback){
        // validate the input arguments
        if(typeof callback !== 'function'){
            throw new Error("callback function cannot be null!");
        }
        ...
        var url = '/_percolator/'+index+'/'+percolatorId;
        this._request(url, {
            method: 'GET'
        }, function (err, res) {
            if (err) { return callback(err, null, res), undefined; }
